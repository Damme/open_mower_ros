...
std::mutex active_subscribers_mutex;
...
int main(int argc, char **argv) {
    ...
    // 1 Hz is plenty for discovering sensor topics; every getTopics() is an
    // XML-RPC round trip to rosmaster, which at 10 Hz cost ~10% CPU on a Pi Zero 2.
    ros::Rate sensor_check_rate(1.0);
    ...
    while (ros::ok()) {
        ...
        std::for_each(topics.begin(), topics.end(), [&](const ros::master::TopicInfo &item) {
            if (boost::regex_match(item.name, topic_regex)) {
                ...
                {
                    std::unique_lock<std::mutex> lk(mqtt_callback_mutex);
                    known = found_sensors.count(item.name) != 0;
                }
                // active_subscribers is also modified from the info callback (spinner thread)
                std::unique_lock<std::mutex> sub_lk(active_subscribers_mutex);
                if (!known && active_subscribers.count(item.name) == 0) {
                    ...
                    active_subscribers[item.name] = n->subscribe<xbot_msgs::SensorInfo>(item.name, 1,
                        [topic = item.name](const xbot_msgs::SensorInfo::ConstPtr &msg) {
                            ROS_INFO_STREAM("got sensor info for sensor on topic " << msg->sensor_name
                                            << " on topic " << topic);
                            {
                                std::unique_lock<std::mutex> lk(mqtt_callback_mutex);
                            ...
                            {
                                std::unique_lock<std::mutex> sub_lk(active_subscribers_mutex);
                                active_subscribers.erase(topic);
                            }
                            // Subscribe for data
                            subscribe_to_sensor(topic);
