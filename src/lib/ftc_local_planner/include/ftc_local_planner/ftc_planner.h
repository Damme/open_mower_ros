...
namespace ftc_local_planner
{
    ...
    private:
        ...
        bool recoveryActive(const geometry_msgs::PoseStamped &pose,
            geometry_msgs::TwistStamped &cmd_vel);
        void skipAhead(double dist);
        int recovery_phase = 0;              // 0 none, 1 release, 2 reverse, 3 skip
        ros::Time recovery_phase_start;
        Eigen::Vector2d stuck_ref_pos{0.0, 0.0};
        ros::Time stuck_ref_time;
